...
The `COCA.getOC` function is used to obtain the operating characteristics of stage 1 (selection and expected sample size) and stage 2 (power, GP, SR, OSR, and expected sample size) of COCA. 
...
```{r, echo = FALSE}
stage1_output <- c(1.44, 83.54, 15.02, 83.54, 43.79)
names(stage1_output) <- c("termination (%)", paste0("dose", 1:2, " (%)"), "selection (%)", "EN")
stage2_output <- c(78.10, 74.19, 69.48, 67.53, 104.6)
...
display.output
```
In stage 1, we have an 83.54\% chance of selecting the correct combination dose as the OBD, with an average sample size of 43.79 patients. In stage 2, the power of our design is 78.10\%, the GP is 74.19\%, the SR is 69.48\%, and the OSR is 67.53\%, with an average sample size of 104.60 patients. In total, the trial requires an average of 148.39 patients (43.79 in stage 1 and 104.60 in stage 2). For illustration, consider using a smaller number of replicates (e.g., `n.simu = 100`).
...
```{r, echo = FALSE}
stage1_output <- c(0.3, 81.6, 18.1, 81.6, 45.88)
names(stage1_output) <- c("termination (%)", paste0("dose", 1:2, " (%)"), "selection (%)", "EN")
stage2_output <- c(78.44, 73.84, 71.78, 69.07, 105.5)
names(stage2_output) <- c("Power (%)", "GP (%)", "SR (%)", "OSR (%)", "EN")
display.output <- list(
    stage1_output = stage1_output, stage2_output = stage2_output
  )
