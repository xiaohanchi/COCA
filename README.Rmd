...
To search for the optimal stage 2 sample size from 20 to 30 and get the corresponding design parameters, run:
...
For more accurate calibration, consider increasing `prior.sample` to $10^6$ and setting `n.simu` to $10^4$, though this may require additional computation time. If the power or success rate does not reach the target, increase `n.stage2` and rerun the calibration.
...
`prior.sample` is set to $10^5$ for illustration. To reproduce the results in our paper, please use `prior.sample = 1e6`.
...
```{r, echo = FALSE}
stage1_output <- c(1.56, 83.34, 15.10, 83.34, 43.8)
names(stage1_output) <- c("termination (%)", paste0("dose", 1:2, " (%)"), "selection (%)", "EN")
stage2_output <- c(76.94, 73.47, 68.24, 66.72, 104.2)
