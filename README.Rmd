...
display.output <- tibble(
  case = 2, n.stage2 = 38, Ce1 = 0.7710, c0 = 0.84, Power = 0.91, TypeI = 0.10
)
display.output
```
This code may take a while... (Note: In my own implementation, I used high-performance computing clusters and parallel computing to accelerate the process.) For illustration, consider using a smaller number of replicates (e.g., `n.simu = 100`). Once completed, we obtained the optimal stage 2 sample size of 38, along with design cutoffs $C_{e1}=0.7710$ and $c_0=84$, as reported in our paper. 
